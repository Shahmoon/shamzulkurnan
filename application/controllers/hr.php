...
class Hr extends CI_Controller {
	...
	function index($offset = 0){
		...
		foreach ($persons as $person){
			$this->table->add_row($person->emp_no, $person->first_name,$person->last_name, strtoupper($person->gender)=='M'? 'Male':'Female', date('d-m-Y',strtotime($person->birth_date)), date('d-m-Y',strtotime($person->hire_date)), 
				anchor('hr/view/'.$person->emp_no,'view',array('class'=>'view')).' '.
				anchor('hr/update/'.$person->emp_no,'update',array('class'=>'update')).' '.
				anchor('hr/delete/'.$person->emp_no,'delete',array('class'=>'delete','onclick'=>"return confirm('Are you sure want to delete this person?')"))
			);
		}
	...
	function add(){
		...
		$data['message'] = '';
		$data['action'] = site_url('hr/addPerson');
		$data['link_back'] = anchor('hr/index/','Back to list of persons',array('class'=>'back'));
	...
	function update($id){
		...
		$data['message'] = '';
		$data['action'] = site_url('hr/updatePerson');
		$data['link_back'] = anchor('hr/index/','Back to list of persons',array('class'=>'back'));
